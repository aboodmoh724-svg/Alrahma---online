...
export default async function TeacherStudentReportPage({ params }: Props) {
  ...
  const { examScores: es, dailyMetrics: dm, grade, finalScore, achievements, snapshot, autoFeedback, recommendations } = evaluation;
    ...
    <div className="min-h-screen bg-[#FAFAF9] pb-12">
      ...
      <nav className="bg-[#0C5C5E] text-white px-4 py-3 sticky top-0 z-50 shadow-sm print:hidden">
        <div className="mx-auto max-w-[800px] flex items-center justify-between">
          <Link href="/onsite/summer/teacher" className="flex items-center gap-1.5 text-[12px] font-bold text-white/90 hover:text-white transition-colors">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><polyline points="15 18 9 12 15 6"/></svg>
            العودة للوحة المعلم
          </Link>
          <span className="text-[12px] font-semibold text-white/80">معاينة تقرير الطالب</span>
        </div>
      </nav>
      ...
      <div className="mx-auto max-w-[700px] px-4 py-6 print:px-8 print:py-4 print:max-w-none">
        ...
        <header className="text-center mb-6 print:mb-4">
          ...
          <div className="inline-block bg-[#0C5C5E] text-white rounded-lg px-4 py-1.5 text-[13px] font-bold print:text-[11px]">
            تقرير تقدم الطالب - الفترة الأولى
          </div>
        </header>
        ...
        <section className="bg-white rounded-xl border border-[#E5E3DF] p-4 mb-4">
          <h2 className="text-[14px] font-bold font-heading text-[#0C5C5E] mb-3">📊 الأداء الأكاديمي</h2>
          <div className={`grid gap-2 ${subjectCards.length === 2 ? "grid-cols-2" : "grid-cols-3"}`}>
            {subjectCards.map((card, i) => {
              ...
              return (
                <div key={i} className="rounded-lg border p-3 text-center" style={{ borderColor: g.color + "30", backgroundColor: g.bgColor }}>
                  <p className="text-[20px] mb-0.5">{card.icon}</p>
                  <p className="text-[10px] text-[#9CA3AF]">{card.label}</p>
                  <p className="text-[22px] font-extrabold font-heading mt-0.5" style={{ color: g.color }}>
                    {card.score % 1 === 0 ? card.score : card.score.toFixed(1)}
                  </p>
                  <p className="text-[10px] font-semibold mt-0.5" style={{ color: g.color }}>{g.label}</p>
        ...
        {/* ═══ 7. TEACHER FEEDBACK ═══ */}
        <section className="bg-white rounded-xl border border-[#E5E3DF] p-4 mb-4">
          <h2 className="text-[14px] font-bold font-heading text-[#0C5C5E] mb-3">💬 ملاحظات المعلم</h2>
        ...
        {recommendations.length > 0 && (
          <section className="bg-white rounded-xl border border-[#E5E3DF] p-4 mb-4">
            <h2 className="text-[14px] font-bold font-heading text-[#0C5C5E] mb-3">💡 التوصيات</h2>

...
function relativeTime(date: Date): string {
  ...
  if (diff === 1) return "قبل دقيقة";
  if (diff === 2) return "قبل دقيقتين";
  if (diff <= 10) return `قبل ${diff} دقائق`;
  if (diff <= 59) return `قبل ${diff} دقيقة`;
  const hrs = Math.floor(diff / 60);
  if (hrs === 1) return "قبل ساعة";
  if (hrs === 2) return "قبل ساعتين";
  if (hrs <= 10) return `قبل ${hrs} ساعات`;
  if (hrs <= 23) return `قبل ${hrs} ساعة`;
...
export default async function OnsiteSummerTeacherDashboard({ searchParams }: DashboardPageProps) {
  ...
  const sn = ["أح", "اث", "ثل", "أر", "خم", "جم", "سب"];
  while (c <= today) {
    const dow = getLocalDayOfWeek(c);
    if (dow !== 1) {
      const ds = toLocalDateKey(c);
      const isT = ds === todayStr;
      availableDates.push({ dateKey: ds, label: isT ? `اليوم - ${dn[dow]} (${ds})` : `${dn[dow]} (${ds})`, isToday: isT, dayName: sn[dow], dayNum: ds.slice(8) });
    }
  ...
  const estMin = Math.ceil(remaining * 1.5);

  // Activity
  const activity = students
    .flatMap((st) => st.summerReports.map((r) => ({ name: st.fullName, status: r.status as string, dateKey: r.dateKey, createdAt: r.createdAt })))
  ...
  const timeline = recentDays.slice(0, 7).reverse().map((day) => {
    ...
    const status = future ? "future" : isToday ? (done ? "complete" : "current") : done ? "complete" : "incomplete";
    return { ...day, status };
  });
  ...
  if (durations.length > 0) avgMin = Math.round(durations.reduce((a, b) => a + b, 0) / durations.length);
  ...
  // Sort
  const pending = [...meta.filter((s) => !s.reportForSelectedDate)].sort((a, b) => {
  ...
  const completed = meta.filter((s) => Boolean(s.reportForSelectedDate));
  ...
  const gamif = (() => {
    ...
    if (pct === 100) return "✅ تم إكمال رصد جميع الطلاب";
    if (pct >= 90) { const r = remaining === 1 ? "طالب واحد" : remaining === 2 ? "طالبان" : `${remaining} طلاب`; return `🏆 بقي ${r} فقط!`; }
    if (pct >= 61) return `🎉 بقي ${remaining} — أقل من ${estMin} دقائق`;
    if (pct >= 31) return `⚡ أكثر من النصف! بقي ${remaining}`;
    if (pct >= 1) return `🔥 أنجزت ${filled} من ${students.length}`;
    return null;
  })();
        ...
        <div className="mx-auto max-w-[1200px] px-4 sm:px-6">
          <div className="flex items-center justify-between h-14">
            <div className="flex items-center gap-2.5">
              <Image src="/images/alrahma_tahfeez_logo.png" alt="تحفيظ الرحمة" width={32} height={32} className="h-8 w-8 rounded-lg object-contain" />
              <div>
                <h1 className="text-[14px] font-bold font-heading leading-tight">تحفيظ الرحمة</h1>
                <p className="text-[10px] text-white/50 hidden sm:block">الدورة الصيفية</p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              <span className="text-[11px] text-white/70 hidden sm:block">{teacher.fullName}</span>
              <div className="h-8 w-8 rounded-full bg-white/15 flex items-center justify-center text-[12px] font-bold border border-white/20">
                {teacher.fullName?.charAt(0) || "أ"}
              </div>
              <LogoutButton redirectUrl="/onsite/summer" className="h-8 w-8 rounded-lg bg-transparent hover:bg-white/10 flex items-center justify-center text-white/50 hover:text-white transition-colors duration-150" />
            </div>
          </div>
        </div>
        <div className="h-px w-full bg-[#1A8A8D]" />
      </header>

      <main className="mx-auto max-w-[1200px] px-4 sm:px-6 pt-4 space-y-3">

        {/* ── 1. Hero: Progress is #1 visual focus ── */}
        <div className="bg-[#EDF5F4] rounded-xl p-4 flex items-center gap-4">
          <div className="flex-1 min-w-0">
            <h2 className="text-[16px] sm:text-[18px] font-bold font-heading text-[#0C5C5E] truncate">
              {greeting} أستاذ {name1}
            </h2>
            {gamif && <p className="text-[12px] text-[#374151] font-semibold mt-0.5">{gamif}</p>}
            {students.length > 0 && (
              <div className="mt-2 flex items-center gap-2">
                ...
                <span className="text-[14px] font-extrabold text-[#0C5C5E] shrink-0 tabular-nums" dir="ltr">{pct}%</span>
              </div>
            )}
          </div>
          <CircularProgress percentage={pct} size={72} strokeWidth={6} filledCount={filled} totalCount={students.length} />
        </div>
        ...
        <div className="flex items-center gap-1.5 overflow-x-auto scrollbar-hide">
          {recentDays.map((day) => {
            ...
            return (
              <Link key={day.dateKey} href={day.isToday ? "/onsite/summer/teacher" : `/onsite/summer/teacher?dateKey=${day.dateKey}`}
                className={`shrink-0 flex flex-col items-center justify-center w-[44px] h-[48px] rounded-lg transition-all duration-150 border ${
                  sel ? "bg-[#0C5C5E] text-white border-[#0C5C5E] shadow-sm" : "bg-white text-[#374151] border-[#E5E3DF] hover:border-[#0C5C5E]/30"
                }`}>
                <span className={`text-[9px] font-medium ${sel ? "text-white/60" : "text-[#9CA3AF]"}`}>{day.dayName}</span>
                <span className={`text-[14px] font-bold ${sel ? "text-white" : "text-[#1F2937]"}`}>{day.dayNum}</span>
          ...
          {availableDates.length > 7 && (
            <form method="GET" className="shrink-0 flex items-center gap-1">
              <select name="dateKey" defaultValue="" className="h-[48px] rounded-lg border border-[#E5E3DF] bg-white px-2 text-[10px] font-semibold text-[#9CA3AF] outline-none focus:border-[#0C5C5E] cursor-pointer">
                <option value="">سابق</option>
                {availableDates.slice(7).map((d) => <option key={d.dateKey} value={d.dateKey}>{d.label}</option>)}
              </select>
              <button type="submit" className="h-[48px] rounded-lg bg-[#0C5C5E] px-2 text-white hover:bg-[#0A4D4F] transition-colors duration-150">
        ...
        {pct === 100 && students.length > 0 && (
          <div className="rounded-lg bg-[#ECFDF5] border border-[#A7F3D0] px-4 py-3 text-center">
            <p className="text-[13px] font-bold text-[#065F46]">بارك الله فيك — تم إكمال رصد جميع الطلاب ✅</p>
            <p className="text-[10px] text-[#065F46]/60 mt-0.5">«خيركم من تعلم القرآن وعلمه»</p>
          </div>
        )}

        {/* ── 5. Students (main content) ── */}
        {meta.length === 0 ? (
          <div className="rounded-xl bg-white border border-dashed border-[#E5E3DF] p-10 text-center">
            <svg width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="#D1D5DB" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="mx-auto mb-2"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>
            <p className="text-[13px] font-bold text-[#374151]">لا يوجد طلاب في حلقتك</p>
            <p className="text-[11px] text-[#9CA3AF] mt-0.5">سيظهر طلابك هنا عند إسنادهم من قبل الإدارة</p>
          </div>
        ) : (
          <>
            {/* Pending */}
            {pending.length > 0 && (
              <section>
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2">
                    <h3 className="text-[14px] font-bold font-heading text-[#1F2937]">بانتظار الرصد</h3>
                    <span className="bg-[#0C5C5E]/10 text-[#0C5C5E] text-[10px] font-bold px-1.5 py-0.5 rounded">{pending.length}</span>
                  </div>
                  {firstPending && (
            ...
            {completed.length > 0 && (
              <section>
                <div className="flex items-center gap-2 mb-2">
                  <h3 className="text-[14px] font-bold font-heading text-[#1F2937]">تم الرصد</h3>
                  <span className="bg-[#059669]/10 text-[#059669] text-[10px] font-bold px-1.5 py-0.5 rounded">{completed.length}</span>
                </div>
                <div className="grid gap-2 sm:grid-cols-2">
                  {completed.map((item, i) => (
                    <div key={item.student.id} style={{ animationDelay: `${i * 20}ms` }} className="animate-fadeIn">
                      <TeacherStudentCard
                        student={item.student} selectedDateKey={selectedDateKey} todayStr={todayStr}
                        reportForSelectedDate={item.reportForSelectedDate} lastPresentReport={item.lastPresentReport}
                        missingDateKeys={item.missingDateKeys} attendanceRate={item.attendanceRate}
        ...
        {students.length > 0 && (
          <div className="grid gap-2 lg:grid-cols-2 pt-1">
            ...
            <div className="bg-white/80 rounded-lg border border-[#E5E3DF]/70 p-3">
              <p className="text-[10px] font-semibold text-[#9CA3AF] mb-1.5">تقدم الأسبوع</p>
                ...
                {timeline.map((d, i) => (
                  <div key={i} className="flex-1 flex flex-col items-center gap-px">
                    <div className={`w-full h-5 rounded flex items-center justify-center text-[8px] font-bold ${
                      d.status === "complete" ? "bg-[#059669] text-white"
                        : d.status === "current" ? "bg-[#0C5C5E] text-white"
                        : d.status === "incomplete" ? "bg-[#FEE2E2] text-[#DC2626]"
                        : "bg-[#F3F4F6] text-[#D1D5DB]"
                    }`}>
                      {d.status === "complete" ? "✔" : d.status === "current" ? "◯" : d.status === "incomplete" ? "✗" : "—"}
                    </div>
                    <span className="text-[8px] text-[#D1D5DB]">{d.dayName}</span>
                  </div>
                ))}
              </div>
            ...
            {/* Activity */}
            <div className="bg-white/80 rounded-lg border border-[#E5E3DF]/70 p-3">
              <p className="text-[10px] font-semibold text-[#9CA3AF] mb-1.5">النشاط الأخير</p>
              {activity.length === 0 ? (
                <p className="text-[10px] text-[#D1D5DB] py-2 text-center">ابدأ برصد أول طالب</p>
              ) : (
                    ...
                    <div key={i}>
                      <div className="flex items-center justify-between py-1">
                        <span className="text-[11px] text-[#6B7280] truncate">
                          {a.status === "ABSENT" ? "❌" : "✅"}{" "}
                          {a.status === "ABSENT" ? `غياب ${a.name}` : `تقرير ${a.name}`}
                        </span>
                        <span className="text-[9px] text-[#D1D5DB] shrink-0 mr-2">{relativeTime(new Date(a.createdAt))}</span>
                      </div>
                      {i < activity.length - 1 && <div className="border-b border-[#F3F4F6]" />}
                    </div>
                  ))}
                </div>
              )}

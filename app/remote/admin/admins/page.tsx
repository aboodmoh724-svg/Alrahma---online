    ...
    <main className="min-h-screen bg-[#f6efe3] px-4 py-6 text-[#0a3f2a]" dir="rtl">
      <div className="mx-auto max-w-7xl space-y-6">
          ...
          <div className="relative flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
            <div>
              ...
              <p className="mt-4 max-w-3xl text-sm leading-8 text-white/72">
                من هنا تضيف الإداري مباشرة داخل قاعدة المنصة المحلية. النظام سينشئ المعرّف تلقائياً، ويمكنك إعطاء صلاحية المالية أو سحبها من الحساب.
              </p>
            </div>
            <div className="flex flex-wrap gap-2">
              <Link href="/remote/admin/supervisors" className="rounded-full bg-[#f2d18a] px-5 py-3 text-sm font-black text-[#0a3f2a]">
                إدارة المشرفين
              </Link>
        ...
        <section className="grid gap-5 lg:grid-cols-[0.9fr_1.6fr]">
          <form action={createRemoteAdmin} className="rounded-[2rem] border border-[#d8bf83] bg-white p-5 shadow-sm">
            ...
            <div className="mt-5 grid gap-3">
              <input name="fullName" required placeholder="اسم الإداري" className="rounded-2xl border border-[#d8bf83] px-4 py-3 text-sm" />
              ...
              <ConfirmSubmitButton
                ...
                className="rounded-2xl bg-[#0a3f2a] px-5 py-3 text-sm font-black text-white"
              >
                إضافة الإداري
              </ConfirmSubmitButton>
            </div>
          ...
          <section className="rounded-[2rem] border border-[#d8bf83] bg-white p-5 shadow-sm">
            <div className="flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
              <div>
                ...
                <p className="mt-2 text-sm text-[#0a3f2a]/60">تعديل البيانات والصلاحيات من هنا مباشرة.</p>
              </div>
                ...
                <article key={admin.id} className="rounded-2xl border border-[#e7d7b4] bg-[#fffaf4]/40 p-4">
                  <form action={updateRemoteAdmin} className="grid gap-3">
                    ...
                    <div className="grid gap-3 md:grid-cols-2">
                      <label className="grid gap-1 text-xs font-black text-[#0a3f2a]/60">
                        الاسم
                        <input name="fullName" defaultValue={admin.fullName} className="rounded-xl border border-[#d8bf83] bg-white px-3 py-2 text-sm text-[#0a3f2a]" />
                      </label>
                      <label className="grid gap-1 text-xs font-black text-[#0a3f2a]/60">
                        البريد
                        <input name="email" type="email" defaultValue={admin.email} className="rounded-xl border border-[#d8bf83] bg-white px-3 py-2 text-sm text-[#0a3f2a]" />
                      </label>
                      <label className="grid gap-1 text-xs font-black text-[#0a3f2a]/60">
                        {"رقم واتساب التنبيهات"}
                        <input name="whatsapp" dir="ltr" defaultValue={admin.whatsapp || ""} placeholder="+905xxxxxxxxx" className="rounded-xl border border-[#d8bf83] bg-white px-3 py-2 text-left text-sm text-[#0a3f2a]" />
                      </label>
                      <label className="grid gap-1 text-xs font-black text-[#0a3f2a]/60">
                        كلمة مرور جديدة اختيارية
                        <input name="password" placeholder="اتركها فارغة إن لم ترد تغييرها" className="rounded-xl border border-[#d8bf83] bg-white px-3 py-2 text-sm text-[#0a3f2a]" />
                      </label>
                      <p className="rounded-xl bg-white px-3 py-2 text-xs leading-6 text-[#0a3f2a]/60">
                    ...
                    <div className="flex flex-wrap gap-3">
                      ...
                      <span className={`rounded-full px-4 py-2 text-xs font-black ${admin.canAccessFinance ? "bg-[#0a3f2a] text-white" : "bg-stone-100 text-stone-600"}`}>
                    ...
                    <div className="flex flex-wrap gap-2">
                      <ConfirmSubmitButton
                        ...
                        className="w-fit rounded-xl bg-[#0f5a35] px-5 py-2 text-xs font-black text-white"
                      >
                        حفظ التعديلات
                      </ConfirmSubmitButton>
                      <ConfirmSubmitButton

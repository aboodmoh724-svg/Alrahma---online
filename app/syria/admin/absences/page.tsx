...
function syriaAbsenceNotice(input: { studentName: string; reportDate: string }) {
  return (
...
async function sendTodayAbsenceWhatsApp() {
  ...
  if (!adminId || !isWhatsAppConfigured("ONSITE_SYRIA")) {
    return;
  }
...
async function sendOneAbsenceWhatsApp(formData: FormData) {
  ...
  if (!adminId || !reportId || !isWhatsAppConfigured("ONSITE_SYRIA")) {
    return;
  }
...
export default async function OnsiteAdminAbsencesPage({ searchParams }: PageProps) {
  ...
  const reportDate = formatIstanbulDateEnglish(start);
  const whatsappReady = isWhatsAppConfigured("ONSITE_SYRIA");

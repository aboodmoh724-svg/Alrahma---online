...
export default function OnsiteAdminTeachersPage() {
  ...
  const fetchTeachers = async () => {
    try {
      setLoading(true);
      const res = await fetch("/api/teachers?studyMode=ONSITE_SYRIA", {
        cache: "no-store",
      });
      const data = await res.json();
      const list = Array.isArray(data.teachers) ? (data.teachers as Teacher[]) : [];
      setTeachers(list.filter((teacher) => teacher.studyMode === "ONSITE_SYRIA"));
    } catch (error) {
      console.error("FETCH ONSITE_SYRIA TEACHERS ERROR =>", error);
      setTeachers([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      console.error("CREATE ONSITE_SYRIA TEACHER SUBMIT ERROR =>", error);
  ...
  const handleUpdateTeacher = async (teacherId: string) => {
    try {
      ...
      console.error("UPDATE ONSITE_SYRIA TEACHER ERROR =>", error);
  ...
  const handleDeleteTeacher = async (teacher: Teacher) => {
    ...
    try {
      ...
      const res = await fetch("/api/teachers", {
        ...
        body: JSON.stringify({ teacherId: teacher.id, studyMode: "ONSITE_SYRIA" }),
      });
      ...
      console.error("DELETE ONSITE_SYRIA TEACHER ERROR =>", error);

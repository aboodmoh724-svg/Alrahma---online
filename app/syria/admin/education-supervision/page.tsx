      ...
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        <section className="grid gap-3 md:grid-cols-3">
          {[
            ...
            <Link
              key={item.key}
              href={`/syria/admin/education-supervision?track=${item.key}`}
          ...
          <section className="rounded-[2rem] bg-white/88 p-5 shadow-sm ring-1 ring-[#d8bf83]">
            ...
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
              {circlePerformance.map((item) => (
                <Link
                  key={item.circle.id}
                  href={`/syria/admin/education-supervision?view=performance&circleId=${item.circle.id}`}
            ...
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
              {visibleCircles.map((circle) => {
                ...
                return (
                  <Link
                    key={circle.id}
                    href={`/syria/admin/education-supervision?track=${selectedTrack}&circleId=${circle.id}&day=${selectedDay.key}`}
                    className={`rounded-[1.8rem] p-5 ring-1 transition hover:-translate-y-0.5 ${
          ...
          <section className="rounded-[2rem] bg-white/88 p-5 shadow-sm ring-1 ring-[#d8bf83]">
            ...
            <div className="mb-5 flex flex-wrap gap-2">
              {attendanceDays.map((day) => (
                <Link
                  key={day.key}
                  href={`/syria/admin/education-supervision?track=${selectedTrack}&circleId=${selectedCircle.id}&day=${day.key}`}

...
export default function OnsiteAdminImportPage() {
  ...
  useEffect(() => {
    const fetchTeachers = async () => {
      try {
        const res = await fetch("/api/teachers?studyMode=ONSITE_SYRIA", { cache: "no-store" });
        const data = await res.json();
        const list = Array.isArray(data.teachers) ? (data.teachers as Teacher[]) : [];
        const syria = list.filter((t) => t.studyMode === "ONSITE_SYRIA" && t.isActive);
        setTeachers(syria);
        if (!fallbackTeacherId && syria[0]?.id) {
          setFallbackTeacherId(syria[0].id);
        }
      } catch {
        setTeachers([]);
      }
  ...
  const importRows = async () => {
    ...
    for (let i = 0; i < result.rows.length; i += 1) {
      ...
      try {
        const res = await fetch("/api/admin/syria/import-students", {
          method: "POST",

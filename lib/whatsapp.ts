...
export function parentEducationChatGuideWhatsAppMessage(input: {
  studentName: string;
  chatUrl: string;
}) {
  return (
    ...
    `تم تفعيل قناة *المراسلات اليومية مع المعلم* الخاصة بالطالب/ـة *${input.studentName}*.\n\n` +
...
export function teacherEducationChatGuideWhatsAppMessage(input: {
  teacherName?: string | null;
  studentName: string;
  ...
  return (
    `السلام عليكم ورحمة الله وبركاته\n\n` +
    `أستاذنا${input.teacherName ? ` *${input.teacherName}*` : ""}، تم تفعيل قناة المراسلات التعليمية مع ولي أمر الطالب/ـة *${input.studentName}* داخل منصة الرحمة.\n\n` +
    ...
    (input.parentPhone ? `\nرقم ولي الأمر المسجل: ${input.parentPhone}\n` : "") +
